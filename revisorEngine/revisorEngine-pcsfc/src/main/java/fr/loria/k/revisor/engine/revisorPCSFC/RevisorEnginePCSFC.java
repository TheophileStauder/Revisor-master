...
import fr.loria.k.revisor.engine.revisorPCSFC.pclc.Formula;
...
public class RevisorEnginePCSFC extends AbstractRevisorEngine {
	...
	public PCSFCFormula revise(PCSFCFormula psi, PCSFCFormula mu, double epsilon) {
		...
		//Converting PCSFC_Formula from console mode to PCSFC classes
		PCSFCFormula psiPCSFC = psi.toPCSFC();
		PCSFCFormula muPCSFC = mu.toPCSFC();

		//Convert PCSFC formula to PCLC formula
		Formula psiPCLC = psiPCSFC.toPCLC();
		Formula muPCLC = muPCSFC.toPCLC();
